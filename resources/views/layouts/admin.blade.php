...
<body class="bg-slate-50 text-slate-800 flex min-h-screen">
    ...
    <div x-data="{ sidebarOpen: true }" class="flex w-full min-h-screen">
        ...
        <!-- Sidebar Navigation -->
        <aside class="bg-white border-r border-slate-200/80 w-64 flex-shrink-0 transition-all duration-300 flex flex-col justify-between"
            :class="sidebarOpen ? 'block' : 'hidden md:block md:w-20'">
            
            <div class="space-y-8">
                <!-- Branding Header -->
                <div class="h-20 flex items-center px-6 border-b border-slate-200/80 bg-white">
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center group">
                        <img src="{{ asset('images/logo.png') }}" alt="Shopee Logo" class="w-10 h-10 rounded-xl shadow-lg shadow-emerald-900/10 object-cover">
                        <span x-show="sidebarOpen" class="ml-3 text-lg font-extrabold text-slate-900 tracking-tight">
                            Shopee <span class="text-orange-500">Admin</span>
                        </span>
                    </a>
                </div>
                ...
                <nav class="px-4 space-y-2.5">
                    ...
                    <!-- Store Globe link -->
                    <a href="{{ route('home') }}" class="flex items-center px-4 py-3 rounded-xl transition font-medium text-sm text-slate-600 hover:bg-slate-50 hover:text-emerald-600">
            ...
            <div class="p-4 border-t border-slate-100 bg-slate-50/50">
                <div class="flex items-center space-x-3 mb-3">
                    <div class="w-8 h-8 rounded-lg bg-emerald-500 text-white flex items-center justify-center font-bold text-xs">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full py-2 bg-white border border-slate-200/80 hover:bg-rose-50 hover:text-rose-600 text-slate-650 text-xs font-bold rounded-lg transition flex items-center justify-center shadow-sm">
        ...
        <!-- Main Layout Area -->
        <div class="flex-1 flex flex-col min-h-screen bg-slate-50/50 overflow-hidden">
            <!-- Header bar -->
            <header class="bg-white h-20 flex items-center justify-between px-6 sm:px-8 border-b border-slate-200/80 sticky top-0 z-30 shadow-sm">
                <div class="flex items-center">
                    <button @click="sidebarOpen = !sidebarOpen" class="p-2 bg-slate-100 border border-slate-200 text-slate-500 hover:text-emerald-600 rounded-lg transition mr-4 focus:outline-none">
                        <i class="fas fa-bars"></i>
                    </button>
                    <h2 class="font-extrabold text-slate-900 text-lg tracking-tight">@yield('page_title', 'Dashboard')</h2>
                </div>

                <div class="flex items-center space-x-4">
                    <span class="text-slate-500 text-xs hidden sm:inline">Current System Date: {{ now()->format('M d, Y') }}</span>
                    <a href="{{ route('home') }}" class="px-4 py-2 bg-white border border-slate-200 text-slate-755 text-xs font-bold rounded-lg hover:bg-slate-50 hover:text-emerald-600 transition flex items-center shadow-sm">
    ...
    <div x-data="{ 
         ...
         class="fixed bottom-5 right-5 z-50 max-w-sm w-full bg-white border rounded-2xl shadow-2xl flex items-center p-4 border-slate-200 overflow-hidden text-slate-800"
         style="display: none;">
        
        <div class="flex-shrink-0 mr-3">
            <template x-if="type === 'success'">
                <div class="w-10 h-10 rounded-xl bg-green-500/10 text-green-600 flex items-center justify-center border border-green-500/20">
                    <i class="fas fa-check-circle text-lg"></i>
                </div>
            </template>
            <template x-if="type === 'error'">
                <div class="w-10 h-10 rounded-xl bg-red-500/10 text-red-600 flex items-center justify-center border border-red-500/20">
        ...
        <div class="flex-1">
            <p class="text-sm font-bold text-slate-900" x-text="type === 'success' ? 'Success' : 'Error'"></p>
