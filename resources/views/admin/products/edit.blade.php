...
@section('content')
    <div class="space-y-6 max-w-4xl mx-auto">
        <div class="flex items-center justify-between pb-4 border-b border-slate-800">
            <div>
                <a href="{{ route('admin.products.index') }}" class="text-xs font-bold text-emerald-400 hover:underline flex items-center mb-2">
                    <i class="fas fa-arrow-left mr-1.5"></i> Back to Inventory
                </a>
                <h1 class="text-2xl font-black text-white tracking-tight">Edit Product: <span class="text-emerald-400">{{ $product->name }}</span></h1>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $product->name) }}" required
                        class="w-full px-4 py-3 bg-slate-900 border border-slate-800 rounded-xl focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 text-sm focus:outline-none text-slate-100 placeholder-slate-600">
                ...
                <div>
                    ...
                    <select id="category_id" name="category_id" required
                        class="w-full px-4 py-3 bg-slate-900 border border-slate-800 rounded-xl focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 text-sm focus:outline-none text-slate-100">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="5" required
                        class="w-full px-4 py-3 bg-slate-900 border border-slate-800 rounded-xl focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 text-sm focus:outline-none text-slate-100 placeholder-slate-600">{{ old('description', $product->description) }}</textarea>
                    ...
                    @if($product->image)
                        <div class="flex items-center space-x-4 p-4 bg-slate-900 border border-slate-800 rounded-2xl w-max">
                            ...
                            <div>
                                ...
                                <a href="{{ $product->image }}" target="_blank" class="text-[10px] text-emerald-400 hover:underline truncate max-w-xs block font-mono">{{ $product->image }}</a>
                            </div>
                        </div>
                    @endif

                    <input type="file" id="image" name="image" accept="image/*"
                        class="w-full px-4 py-3 bg-slate-900 border border-slate-800 rounded-xl focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 text-sm focus:outline-none text-slate-400 file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-slate-800 file:text-emerald-400 hover:file:bg-slate-700">
